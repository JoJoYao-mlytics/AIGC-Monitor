/**
 * U-CAR 試車專區 AIGC 監控範例
 * 
 * 執行方式：
 * node example-ucar.js
 * 
 * 或使用環境變數：
 * SITE_ID=ucar BASE_URL=https://roadtest.u-car.com.tw/roadtest/articles DEBUG=true node example-ucar.js
 */
...
process.env.BASE_URL = process.env.BASE_URL || 'https://roadtest.u-car.com.tw/roadtest/articles';
process.env.DEBUG = process.env.DEBUG || 'true'; // 開啟 debug 模式以查看瀏覽器

async function main() {
    console.log('='.repeat(80));
    console.log('📊 U-CAR 試車專區 AIGC 功能監控範例');
    console.log('='.repeat(80));
    console.log();
    ...
    try {
        console.log('🚀 開始監控 U-CAR AIGC 功能...');
        ...
        console.log(`  ${results.results.news_article_loaded ? '✅' : '❌'} 試車報告載入`);
        ...
        if (results.screenshots.length > 0) {
            ...
            console.log(`   儲存位置: reports/ucar/${results.timestamp.split('T')[0]}/`);
            console.log();
        }
        ...
        console.log(`   • JSON: reports/ucar/${date}/ucar-${timestamp}-results.json`);
        console.log(`   • HTML: reports/ucar/${date}/ucar-${timestamp}-report.html`);
        console.log('='.repeat(80));
        console.log();
