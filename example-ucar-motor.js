 ...
 * 
 * 執行方式：
 * node example-ucar-motor.js
 * 
 * 或使用環境變數：
 * SITE_ID=ucar-motor BASE_URL=https://motor.u-car.com.tw/motor/articles DEBUG=true node example-ucar-motor.js
 */

const AIGCMonitor = require('./src/monitor');

// 設置環境變數
process.env.SITE_ID = 'ucar-motor';
process.env.BASE_URL = process.env.BASE_URL || 'https://motor.u-car.com.tw/motor/articles';
process.env.DEBUG = process.env.DEBUG || 'true'; // 開啟 debug 模式以查看瀏覽器

async function main() {
    console.log('='.repeat(80));
    console.log('🏍️  U-CAR 機車區 AIGC 功能監控範例');
    console.log('='.repeat(80));
    console.log();
    ...
    try {
        console.log('🚀 開始監控 U-CAR 機車區 AIGC 功能...');
        ...
        console.log(`  ${results.results.news_article_loaded ? '✅' : '❌'} 機車文章載入`);
        ...
        if (results.screenshots.length > 0) {
            ...
            console.log(`   儲存位置: reports/ucar-motor/${results.timestamp.split('T')[0]}/`);
            console.log();
        }
        ...
        console.log(`   • JSON: reports/ucar-motor/${date}/ucar-motor-${timestamp}-results.json`);
        console.log(`   • HTML: reports/ucar-motor/${date}/ucar-motor-${timestamp}-report.html`);
        console.log('='.repeat(80));
        console.log();
