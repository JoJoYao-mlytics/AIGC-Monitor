 ...
 * 
 * 執行方式：
 * node example-cnyes.js
 * 
 * 或使用環境變數：
 * SITE_ID=cnyes BASE_URL=https://www.cnyes.com/ DEBUG=true node example-cnyes.js
 */
...
process.env.BASE_URL = process.env.BASE_URL || 'https://www.cnyes.com/';
process.env.DEBUG = process.env.DEBUG || 'true'; // 開啟 debug 模式以查看瀏覽器

async function main() {
    console.log('='.repeat(80));
    console.log('📊 CNYES (鉅亨網) AIGC 功能監控範例');
    console.log('='.repeat(80));
    console.log();
    ...
    try {
        console.log('🚀 開始監控 CNYES AIGC 功能...');
        ...
        console.log(`  ${results.results.news_article_loaded ? '✅' : '❌'} 文章頁面載入`);
        console.log(`  ${results.results.aigc_resources_loaded ? '✅' : '❌'} AIGC 資源載入`);
        console.log(`  ${results.results.ai_section_found ? '✅' : '⚠️'} AI 區塊存在`);
        ...
        if (results.aigc_verification.metadata_html_loaded) {
            console.log('  ✅ metadata_html API 已載入');
        }
        if (results.aigc_verification.answer_html_loaded) {
            console.log('  ✅ answer_html API 已載入');
        }
        if (results.aigc_verification.questions_html_loaded) {
            console.log('  ✅ questions_html API 已載入');
        }
        if (results.aigc_verification.member_html_loaded) {
            console.log('  ✅ member_html API 已載入');
        }
        if (results.aigc_verification.questions_ajax_loaded) {
            console.log('  ✅ questions_ajax API 已載入');
        }
        ...
        if (results.screenshots.length > 0) {
            ...
            console.log(`   儲存位置: reports/${results.timestamp.split('T')[0]}/`);
            console.log();
        }
        ...
        console.log('='.repeat(80));
        console.log('📄 完整報告已儲存:');
        console.log(`   • JSON: reports/${results.timestamp.split('T')[0]}/cnyes-${results.timestamp.split('T')[1].split('.')[0].replace(/:/g, '-')}-results.json`);
        console.log(`   • HTML: reports/${results.timestamp.split('T')[0]}/cnyes-${results.timestamp.split('T')[1].split('.')[0].replace(/:/g, '-')}-report.html`);
        console.log('='.repeat(80));
        console.log();
