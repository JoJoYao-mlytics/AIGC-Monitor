...
test.describe('am.u-car.com.tw AIGC verification', () => {
  test('End-to-end flow: AM Article -> AIGC Question -> Answer page', async ({ context, page }, testInfo) => {
    ...
    page.on('console', msg => {
      const text = msg.text();
      if (text.includes('AIGC') || text.includes('aigc') || text.includes('mlytics')) {
        consoleLogs.push({ type: msg.type(), text });
      }
    });

    // STEP 1: Navigate to AM (售後市場) main page
    console.log('STEP 1: Opening AM (售後市場) page...');
    await page.goto('https://am.u-car.com.tw/am', { 
      waitUntil: 'domcontentloaded',
      timeout: 30000
    });
    await page.waitForTimeout(2000); // Wait for dynamic content
    await testInfo.attach('01-am-main-page.png', { 
      body: await page.screenshot({ fullPage: false }), 
      contentType: 'image/png' 
    });

    // STEP 2: Find and click first AM article
    console.log('STEP 2: Finding and clicking AM article...');
    
    // Locate any AM article link - look for article list
    const articleLink = page.locator('a[href*="/am/article/"]').first();
    await expect(articleLink).toBeVisible({ timeout: 10000 });
    
    const articleHref = await articleLink.getAttribute('href');
    const articleText = (await articleLink.textContent())?.trim() || '';
    console.log(`  Article title: "${articleText}"`);
    console.log(`  Article URL: ${articleHref}`);
    
    expect(articleHref).toContain('/am/article/');
    
    await articleLink.click();
    await page.waitForLoadState('domcontentloaded');
    await page.waitForTimeout(2000);
    await testInfo.attach('02-am-article-clicked.png', { 
      body: await page.screenshot({ fullPage: false }), 
      contentType: 'image/png' 
    });
    ...
    // The page title should contain "U-CAR售後" or "U-CAR"
    expect(pageTitle).toContain('U-CAR');
    expect(pageTitle.toLowerCase()).toContain('售後');
    ...
    for (let i = 0; i < questionsCount; i++) {
      const questionText = (await aigcQuestions.nth(i).textContent())?.trim() || '';
    ...
    // Verify "Answer / Powered by Mlytics AI" badge
    ...
    const dataSourceHeading = answerPage.locator('h3, h4, h5').filter({ 
    ...
    // Verify data source has links to source articles
    ...
    // List data source links
    for (let i = 0; i < Math.min(sourceLinksCount, 5); i++) {
      const linkText = await dataSourceLinks.nth(i).textContent();
      const linkHref = await dataSourceLinks.nth(i).getAttribute('href');
      console.log(`    Source ${i + 1}: ${linkText?.trim()} (${linkHref})`);
    }
    ...
    const bottomAigcHeading = answerPage.locator('h2, h3, h4').filter({ 
    ...
    const relatedQuestions = answerPage.locator('h4, h5').filter({ hasText: /.{10,}/ });
    ...
    uniqueConsoleLogs.slice(0, 20).forEach(logText => {
      console.log(`  ${logText}`);
    });

    // Check for key AIGC initialization messages
    const hasAigcInitLog = consoleLogs.some(log => 
      log.text.includes('AIGC') || log.text.includes('aigc')
    );
    console.log(`  AIGC logs captured: ${hasAigcInitLog ? 'Yes' : 'No'}`);

    // Save comprehensive test logs
    const testSummary = {
      testName: 'am.u-car.com.tw AIGC E2E Test',
      timestamp: new Date().toISOString(),
      article: {
        title: pageTitle,
        url: page.url(),
        linkText: articleText
      },
      question: {
        text: questionText,
        answerPageTitle,
