...
test.describe('U-CAR 機車區 AIGC 功能監控', () => {
    test('應該成功完成所有 AIGC 功能檢查', async () => {
        ...
        expect(results.site).toBe('ucar-motor');
    ...
    test('應該驗證完整的 AI 問答流程', async () => {
        ...
        const checks = {
            ...
            '機車文章已載入': results.results.news_article_loaded,
    ...
    test('應該驗證機車區專屬內容', async () => {
        const monitor = new AIGCMonitor();
        const results = await monitor.run();
        
        // 驗證基礎 URL 是機車區
        expect(results.baseUrl).toContain('motor.u-car.com.tw');
        
        // 驗證文章 URL 格式
        const articleUrl = results.api_requests.find(req => req.url.includes('/motor/article/'));
        console.log('\n🏍️ 機車區專屬驗證:');
        console.log(`基礎 URL: ${results.baseUrl}`);
        if (articleUrl) {
            console.log(`✅ 確認為機車區文章: ${articleUrl.url}`);
        }
