/**
 * U-CAR 車壇新聞 AIGC 監控範例
 * 
 * 執行方式：
 * node example-ucar-news.js
 * 
 * 或使用環境變數：
 * SITE_ID=ucar-news BASE_URL=https://news.u-car.com.tw/news/articles DEBUG=true node example-ucar-news.js
 */
...
process.env.BASE_URL = process.env.BASE_URL || 'https://news.u-car.com.tw/news/articles';
process.env.DEBUG = process.env.DEBUG || 'true'; // 開啟 debug 模式以查看瀏覽器

async function main() {
    console.log('='.repeat(80));
    console.log('🗞️  U-CAR 車壇新聞 AIGC 功能監控範例');
    console.log('='.repeat(80));
    console.log();
    ...
    try {
        console.log('🚀 開始監控 U-CAR 車壇新聞 AIGC 功能...');
        ...
        console.log(`  ${results.results.news_article_loaded ? '✅' : '❌'} 車壇新聞文章載入`);
        ...
        const mlyticsRequests = results.api_requests.filter(req => 
            req.url.includes('mlytics.com') && !req.url.includes('api.aigc.mlytics.com')
        );
        if (mlyticsRequests.length > 0) {
            console.log('🌐 Mlytics 相關請求:');
        ...
        if (results.screenshots.length > 0) {
            ...
            console.log(`   儲存位置: reports/ucar-news/${results.timestamp.split('T')[0]}/`);
            console.log();
        }
        ...
        console.log(`   • JSON: reports/ucar-news/${date}/ucar-news-${timestamp}-results.json`);
        console.log(`   • HTML: reports/ucar-news/${date}/ucar-news-${timestamp}-report.html`);
        console.log('='.repeat(80));
        console.log();
