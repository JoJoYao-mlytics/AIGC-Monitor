...
async function checkUcarNewsAIGCFunctionality(page, context, results, config, dateFolder, timestamp) {
    console.log('\n📍 開始檢測 U-CAR 車壇新聞 AIGC 功能...');
    
    try {
        // 注意：此時頁面已經在文章頁面（由 monitor.js 的 navigateToArticle 處理）
        // 步驟 1-3: 驗證文章頁面
        console.log('✅ 步驟 1-3: 驗證車壇新聞文章頁面...');
        const articlePageTitle = await page.title();
        console.log(`📄 車壇新聞文章頁面 title: ${articlePageTitle}`);
        
        // 驗證是否為車壇新聞文章
        const url = page.url();
        if (!url.includes('/news/article/')) {
            console.warn('⚠️ 警告: 頁面 URL 不包含 /news/article/');
        }
        
        await takeScreenshot(page, dateFolder, timestamp, 'ucar-news-article-page', 'success', results, 'ucar-news');
        ...
        await takeScreenshot(page, dateFolder, timestamp, 'ucar-news-ai-section-found', 'success', results, 'ucar-news');
        ...
        await takeScreenshot(newPage, dateFolder, timestamp, 'ucar-news-ai-answer-page', 'success', results, 'ucar-news');
        ...
        await takeScreenshot(newPage, dateFolder, timestamp, 'ucar-news-ai-content-loaded', 'success', results, 'ucar-news');
        ...
        await takeScreenshot(newPage, dateFolder, timestamp, 'ucar-news-data-source', 'success', results, 'ucar-news');
        ...
        await takeScreenshot(newPage, dateFolder, timestamp, 'ucar-news-bottom-ai-section', 'success', results, 'ucar-news');
        
        // 驗證 AIGC 資源載入（步驟 10）
        await verifyAIGCResources(results, config);
        
        console.log('\n✅ U-CAR 車壇新聞 AIGC 功能檢測完成');
        ...
        console.log(`   ✅ AIGC 資源載入: ${results.results.aigc_resources_loaded ? '✅ 通過' : '⚠️ 警告'}`);
        
        // 關閉新頁面
        await newPage.close();
        
    } catch (error) {
        console.error('❌ U-CAR 車壇新聞 AIGC 功能檢測失敗:', error.message);
        results.status = 'failed';
        results.errors.push({
            step: 'ucar_news_aigc_check',
...
async function verifyAIGCResources(results, config) {
    ...
    for (const endpoint of requiredEndpoints) {
        ...
        if (found) {
            ...
            } else if (endpoint === 'tmc_js') {
                console.log('   → Mlytics TMC 腳本已載入');
            }
        } else {
    ...
    // 額外檢查 mlytics.com 和 aigc 相關的請求
    const mlyticsRequests = results.api_requests.filter(req => 
        req.url.includes('mlytics.com') || req.url.includes('aigc')
    );
    
    if (mlyticsRequests.length > 0) {
        console.log(`\n📡 Mlytics & AIGC 相關請求 (共 ${mlyticsRequests.length} 個):`);
        mlyticsRequests.slice(0, 10).forEach(req => {
            const status = req.status >= 200 && req.status < 300 ? '✅' : '❌';
            const urlShort = req.url.split('?')[0].substring(0, 80);
            console.log(`   ${status} [${req.status}] ${urlShort}`);
        });
        if (mlyticsRequests.length > 10) {
            console.log(`   ... 還有 ${mlyticsRequests.length - 10} 個請求`);
        }
    }
    ...
    if (loadedCount > 0) {
        ...
        console.log(`\n✅ AIGC 資源驗證通過 (${loadedCount}/${requiredEndpoints.length})`);
        console.log(`   已載入: ${loadedResources.join(', ')}`);
    } else {
        console.warn('\n⚠️ 未偵測到 AIGC 資源載入');
    }
    ...
    if (expectedMessages.length > 0) {
        ...
        if (foundMessages.length > 0) {
            ...
            console.log(`⚠️ 未找到預期的 Console 訊息`);
        }
    }
}

module.exports = { checkUcarNewsAIGCFunctionality };
