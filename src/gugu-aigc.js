...
/**
 * GUGU 站點專用的 AIGC 功能檢測
 */
async function checkGuguAIGCFunctionality(page, context, results, config, dateFolder, timestamp) {
    console.log('📍 開始檢查 GUGU AIGC 功能...');
    ...
    try {
        ...
        console.log('✅ GUGU AIGC 功能檢查完成');
        
    } catch (error) {
        console.error('❌ GUGU AIGC 功能檢查失敗:', error.message);
        results.status = 'failed';
        results.errors.push({
            message: `GUGU AIGC 檢查失敗: ${error.message}`,
            stack: error.stack,
...
async function verifyAigcResources(page, results, config) {
    ...
    // 改為僅以 AIGC API 是否成功回應作為資源載入判定，不驗證 tracker/CSS/JS
    const apiKeys = [
        'questions_html',
        'answer_html',
        'member_html',
    ...
    try {
        ...
        for (const selector of aiSelectors) {
            try {
                ...
                console.log(`✅ 找到 AI 區塊: ${sectionText?.substring(0, 30)}...`);
        ...
        if (aiSectionFound) {
            ...
            await takeScreenshot(page, dateFolder, timestamp, 'ai-section-found', 'success', results, 'gugu');
            console.log('✅ AI 區塊檢查通過');
        } else {
            console.warn('⚠️ 未找到 AI 區塊');
            await takeScreenshot(page, dateFolder, timestamp, 'ai-section-missing', 'issues', results, 'gugu');
        }
        
    } catch (error) {
        console.error('❌ AI 區塊檢查失敗:', error.message);
        await takeScreenshot(page, dateFolder, timestamp, 'ai-section-error', 'issues', results, 'gugu');
        throw error;
    }
}
    ...
    try {
        ...
        for (const selector of questionSelectors) {
            try {
                ...
                console.log(`✅ 找到 AI 問題: ${questionText?.substring(0, 50)}...`);
        ...
        if (!questionFound) {
            console.warn('⚠️ 未找到 AI 問題連結');
            await takeScreenshot(page, dateFolder, timestamp, 'ai-questions-missing', 'issues', results, 'gugu');
            return;
        }
        
        results.results.ai_questions_available = true;
        await takeScreenshot(page, dateFolder, timestamp, 'ai-questions-found', 'success', results, 'gugu');
        
        // 測試點擊 AI 問題連結
        await testAIQuestionClick(page, context, questionLink, questionText, results, config, dateFolder, timestamp);
        
    } catch (error) {
        console.error('❌ AI 問題檢查失敗:', error.message);
        await takeScreenshot(page, dateFolder, timestamp, 'ai-questions-error', 'issues', results, 'gugu');
        throw error;
    }
}
...
async function testAIQuestionClick(page, context, questionLink, questionText, results, config, dateFolder, timestamp) {
    ...
    try {
        ...
        console.log(`📄 新頁面標題: ${newPageTitle}`);
        ...
        if (titleContainsQuestion) {
            console.log('✅ 新頁面標題與問題匹配');
        } else {
            console.warn(`⚠️ 新頁面標題可能不匹配: "${newPageTitle}" vs "${questionText}"`);
        }
        ...
        await takeScreenshot(newPage, dateFolder, timestamp, 'ai-content-generated', 'success', results, 'gugu');
        
        console.log('✅ AI 問題連結測試通過');
        
        // 關閉新頁面
        await newPage.close();
        
    } catch (error) {
        console.error('❌ AI 問題連結測試失敗:', error.message);
        throw error;
    }
}
...
async function waitForAIContentGeneration(page, results, config) {
    ...
    try {
        ...
        console.log('✅ AI 內容生成等待完成');
        
    } catch (error) {
        console.warn('⚠️ AI 內容生成等待超時:', error.message);
    }
}
...
async function verifyAIContent(page, results, config, dateFolder, timestamp) {
    ...
    try {
        ...
        for (const selector of contentSelectors) {
            try {
                ...
                if (contentText && contentText.trim().length > 50) {
                    contentFound = true;
                    console.log(`✅ 找到 AI 內容: ${contentText.substring(0, 100)}...`);
                    break;
                }
            } catch (error) {
                continue;
        ...
        if (contentFound && keywordFound) {
            console.log('✅ AI 內容驗證通過');
            results.results.ai_content_generated = true;
        } else {
            console.warn('⚠️ AI 內容驗證未完全通過');
            console.warn(`內容找到: ${contentFound}, 關鍵字找到: ${keywordFound}`);
        }
        
    } catch (error) {
        console.error('❌ AI 內容驗證失敗:', error.message);
        await takeScreenshot(page, dateFolder, timestamp, 'ai-content-verification-failed', 'issues', results, 'gugu');
        throw error;
    }
}

module.exports = {
    checkGuguAIGCFunctionality,
    verifyAigcResources,
